...
def chisq_test(series_a, series_b, condition_a, condition_b):
...
for varname, (col, vtype, condition) in baseline_vars.items():
    if vtype == 'cont':
        ...
        p_val = chisq_test(alive[col], died[col], condition, (alive[col] != condition).mode()[0] if condition is not None else None)
...
# SAFETY THRESHOLD: Maximize specificity while Sensitivity ≥ 97.5%
sorted_death = np.sort(death_probs)
for fn_target in range(0, 6):
    thr = sorted_death[fn_target]
    sens = (death_probs >= thr).sum() / n_death
    if sens >= 0.975:
        safety_thr = thr
        safety_sens = sens
        safety_fn = int((death_probs < thr).sum())
        safety_tp = n_death - safety_fn
        tn = int((alive_probs < thr).sum())
        safety_tn = tn
        safety_fp = n_alive - tn
        safety_spec = tn / n_alive
        safety_ppv = safety_tp / (safety_tp + safety_fp)
        safety_npv = safety_tn / (safety_tn + safety_fn)
        break
...
print(f"    Interpretation: Safety-first screening — miss ≤2 deaths at cost of many FP")

# YOUDEN THRESHOLD: Maximize J = Sensitivity + Specificity - 1
...
for thr in thresholds:
    ...
    if j > best_j:
        ...
        youden_ppv = youden_tp / (youden_tp + youden_fp)
        youden_npv = youden_tn / (youden_tn + youden_fn)
